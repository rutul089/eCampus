package rutul.com.ecampus.components.labelui;
...
public class AutoLabelUI extends AutoViewGroup implements Label.OnClickCrossListener, Label.OnLabelClickListener {
    ...
    //-- to get all value as string array
    public List<String> getSelectedStrings() {
        List<String> selection = new LinkedList<>();

        for (int i = 0; i < getChildCount(); i++) {
            selection.add(String.valueOf(getLabels().get(i).getTag()));
        }
        return selection;
    }

    //--To get all value as integer
    public List<Integer> getSelectedLabelId() {
        List<Integer> selectedId = new LinkedList<>();
        for (int i = 0; i < getChildCount(); i++) {
            selectedId.add((Integer) getLabels().get(i).getTag());
        }
        return selectedId;
    }

    //--To get all value as string
